...
export function SponsorProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchSponsorImages = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch sponsors: ${String(response.status)} ${response.statusText}`);
      }
      const data: unknown = await response.json();
      if (!Array.isArray(data)) {
        throw new Error('Expected an array of strings but received a different data structure');
      }
      const images = data.filter((item): item is string => typeof item === 'string');
  ...
  const getRandomSponsorUrl = useCallback((): string => {
    ...
    const availableImages = sponsorImages.filter(img => !usedImagesRef.current.has(img));
    const imagePool = availableImages.length > 0 ? availableImages : sponsorImages;
  ...
  const getRandomSponsorUrls = useCallback((count: number): string[] => {
  ...
  return (
    <SponsorContext value={contextValue}>
      {children}
    </SponsorContext>
  );
}
